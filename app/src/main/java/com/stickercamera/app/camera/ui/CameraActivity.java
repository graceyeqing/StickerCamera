    ...
    private void initEvent() {
        ...
        galleryBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //从手机相册选择图片
                Intent intent = new Intent(CameraActivity.this, PhotoPickActivity.class);
    ...
    protected void onActivityResult(final int requestCode, final int resultCode, final Intent result) {
        ...
        if (requestCode == REQUEST_RESULT_PHOTO) {//接受系统拍照回调
            if (resultCode == Activity.RESULT_OK) {
                //返回选取图片
                ArrayList<ImageInfo> mPickData = (ArrayList<ImageInfo>) result.getSerializableExtra(EXTRA_RESULT_PHOTO_LIST);
                if (mPickData!=null&& mPickData.size() > 0) {
                    Intent newIntent = new Intent(this, PhotoProcessActivity.class);
                    newIntent.putExtra("DATA",mPickData);
                    startActivity(newIntent);
                }
            }
        }
